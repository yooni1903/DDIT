...
//		}
		...
		String path = "https://www.google.co.kr/images/branding/googlelogo/1x/googlelogo_color_272x92dp.png";
		File folder = new File("d:/contents");
		File writeFile = new File(folder, "googlelogo_color_272x92dp.png");
		URL url = new URL(path);
		System.out.println(url);
		
		URLConnection conn = url.openConnection();		// 소켓통신을 위한 소켓을 개방함
		
		try(
			InputStream is =  conn.getInputStream();
			FileOutputStream fos = new FileOutputStream(writeFile);
		){
			byte[] buffer = new byte[1024];	// close할 수 잇는 객체가 아니라서 밑으로 내려옴
			int cnt = -1;
			while((cnt = is.read(buffer)) != -1) {
				fos.write(buffer, 0, cnt);	// 마지막 쓸 때에 buffer자리를 위한 설정이 0,cnt
...
//		 String qualifiedName = "kr/or/ddit/io/another day.txt";	// /가 없다면 상대경로
...
//	     URL url = loader.getResource(qualifiedName);
	     ...
	     try(
	    		InputStream in =  StreamDesc.class.getResourceAsStream(qualifiedName);	// 매체에서 바로 개방한 스트림이기 때문에 1차 스트림이다
