...
public class CryptoUtil {
	...
	public static String md5(String str) throws NoSuchAlgorithmException {
		MessageDigest md = MessageDigest.getInstance("MD5");		// MD5방식으로 암호화하는 instance객체 생성
		md.update(str.getBytes());									// MessageDigest 시작하는 메서드
	...
	public static String encryptAES256(String str, String key) throws UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		...
		System.arraycopy(b, 0, keyBytes, 0, len);	// b배열의 0번째부터 keyBytes에 복사하는데, 0번째부터 len만큼 집어넣으라는 뜻이다.
		...
		//			   부족한 길이만큼 '0'으로 채우거나 임의의 비트로 채워 넣는 것을 말한다.
		//			   PKCSSPadding도 Padding의 종류 중 하나이다.
		...
		// 옵션 종류에 맞게 초기화한다.
		// 옵션 종류 : ENCRYPT_MODE(암호화 모드), DECRYPT_MODE(복호화 모드) 등등
		
		// 초기화 벡터(Initial Vector, IV) ==> 암호문이 패턴화 되지 않도록 사용하는 데이터
		//									   첫 블록을 암호화할 때 사용하는 값
	...
	/**
	 * encryptAES356()메서드로 암호화한 내용을 복호화하는 메서드
	 * @param str 복호화할 문자열 (암호화된 문자열)
	 * @param key 암호키 문자열
	 * @return 복호화된 문자열(원본의 문자열)
	 * @throws UnsupportedEncodingException 
	 * @throws NoSuchPaddingException 
	 * @throws NoSuchAlgorithmException 
	 * @throws InvalidAlgorithmParameterException 
	 ...
	 * @throws IllegalBlockSizeException 
	 */
	public static String decryptAES356(String str, String key) throws UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		String iv = key.substring(0, 16);
		byte[] keyBytes = new byte[16];
		byte[] b = key.getBytes("UTF-8");
		int len = keyBytes.length;
