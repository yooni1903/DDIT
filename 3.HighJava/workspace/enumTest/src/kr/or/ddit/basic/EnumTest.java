...
/*
 	- enum(열거형) ==> 서로 관련있는 상수들의 집합을 나타낸다.
 		...
 		==> 열거형은 클래스처럼 독립된 java 파일에 만들 수 있고, 
 			하나의 java 파일에 클래스와 함께 만들 수 있고,
 			클래스 내부에 내부 클래스처럼 만들 수 있다.
 	- 열거형의 속성 및 메서드
 		1) name() ==> 열거형 상수의 이름을 문자열로 반환한다.
 		2) ordinal() ==> 열거형 상수가 정의된 순서 값(index 값)을 반환한다.
 						 (0부터 시작)
 		3) valueOf("열거형상수명") ==> 지정된 열거형에서 '열거형상수명'과 일치하는 상수를 반환한다.
 		...
 		방법2) ==> 각각의 상수에 임의의 값을 설정할 수 있는 열거형 만들기
 			enum 열거형 이름{
 				...
 				// '값들'이 저장될 변수 선언
 				 ...
 				 ...
 				 // 열거형의 생성자를 만든다.
 				 // 열거형의 생성자는 '열거형상수'에 '값들'을 세팅하는 역할을 수행한다. (열거형 생성자는 묵시적으로 private이다.)
 				 
 				 // '변수명'의 개수는 '값들'의 개수와 같고, 각각의 '값들'의 선언된 변수와 자료형이 같아야 한다.
 				 private 열거형이름(자료형명 변수명, ...){
 				 	위에 선언된 변수들을 초기화하는 작업을 수행한다.
 				 	...
 				 }
 				 
 				 // 구성된 '값들'을 외부에서 불러올 수 있도록
 				 //	getter메서드를 작성한다.
...
public class EnumTest {
	...
	public enum Season{
		...
		//private Season(String span, int startMonth)와 같다.
			this.span = span;
	...
	public static void main(String[] args) {
		...
		if(ConstTest.RED == ConstTest.BLUE){
			System.out.println("...");
		}else{
			System.out.println("???");
		}
...
//		}
		...
		// switch문에서의 비교
		// switch문의 case에 상수 명을 지정할 때는 '열거형 이름'을 생략하고 '상수명'만 지정한다.
		switch(myCnt){
		case ONE:
