...
public class FileTest01 {
	...
	public static void main(String[] args) {
		...
		//		==> 디렉토리와 디렉토리 사이 또는 디렉토리와 파일명 사이의 구분자는 '/'를 사용하거나 '\'를 사용한다.
		
//		File file1 = new File("d:/D_Other/test.txt");	// 구분자로 '/' 사용
		...
		//		==> 중간 부분의 경로가 모두 만들어져 잇어야 마지막 위치의 디렉토리(폴더)를 만들 수 있다.
		
		// - mkdirs() ==> 중간 부분의 경로가 없으면 중간부분의 경로도 같이 만들어준다.
		if(file5.mkdir()){
			System.out.println(file5.getName() + " 만들기 성공~!!!~!~!!~!");
		}else{
			System.out.println(file5.getName() + " 만들기 실패ㅜㅜㅠㅠㅜㅠㅜ");
		}
		System.out.println();
		...
		if(!file6.exists()){
			if(file6.mkdirs()){
				...
				System.out.println(file6.getName() + " 만들기 실패 ㅜㅠㅜㅜ푸ㅜ");
			}
		}
	}
