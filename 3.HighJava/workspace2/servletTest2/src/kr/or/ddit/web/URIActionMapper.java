...
public class URIActionMapper {
	...
	static {
		...
		while(en.hasMoreElements()) {
			String key = en.nextElement(); 		// 키값 1개 가져오기(요청uri)
	...
	// 인수값으로 주어진 uri에 맞는 Action객체를 찾아서 그 Action객체의 인스턴스를 생성해서 반환하는 메서드
	public static IAction getAction(String uri) {
		...
		if(actionMap.containsKey(uri)) {
			try {
				// 문자열로 된 Action객체의 이름을 이용해서 해당 클래스를 메모리에 로딩한다.(메모리에 저장한다)
